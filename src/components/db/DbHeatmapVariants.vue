 ...
 *
 * TODO Consider webgl for fast heatmaps
 *  https://github.com/pyalot/webgl-heatmap
 *  https://observablehq.com/@a10k/webgl-heatmap-temperature-forecasts-for-the-northern-hemi
 *
 * Or Plotly:
 *  https://plotly.com/javascript/heatmap-webgl/
 */
...
export default {
  ...
  methods: {
    ...
    render_V3() {
      ...
      let data = this.data; // apireqdata; // // this.data;
      ...
      let canvas = this.$refs.canvas;
      // TODO Will Need scale function here to map screen coordinates to data point - to show popup, for example.
      let wx = Math.floor(width / serieslength);
      let drawWidth = wx * serieslength;
      let wy = 20;

      height = numseries * wy;

      canvas.width = drawWidth; //W; // ?
      ...
      for (let cy = 0; cy < H; ++cy) {
        for (let cx = 0; cx < W; ++cx) {
          ...
          ctx.fillStyle = c;
          ctx.fillRect(cx * wx, cy * wy, wx, wy);
        }
      }
    ...
    render_V2() {
      ...
      // Single line pixels buffer
      let lineBufferLen = width * 4;
      let lineBuffer = new Array(lineBufferLen).fill(0);
      ...
      for (let cy = 0; cy < H; ++cy) {
        ...
        for (let cx = 0; cx < W; ++cx) {
          let value = data[cy][cx];
          // TODO Note: d3.rgb is expensive call. Consider optimizing
          let c = d3.rgb(d3.interpolateYlOrRd(scaleValue(value)));
          //let c = rgb(0,0,255);
          let pos = scaleX(cx);
          lineValues.push({ v: value, p: pos, c: c });
        }

        // Calculate length in pixels of each data point
        let lineNextPos = lineValues.map(x => x.p);
        lineNextPos.shift(); // remove first
        lineNextPos.push(width); // the last is width
        lineValues.map((x, i) => {
          x.l = lineNextPos[i] - x.p;
        });

        // Step 2: fill line buffer
        let p = 0;
        for (let lv of lineValues) {
          for (let cp = 0; cp < lv.l; cp++) {
            lineBuffer[p++] = lv.c.r;
            lineBuffer[p++] = lv.c.g;
            lineBuffer[p++] = lv.c.b;
            lineBuffer[p++] = 255;
        ...
        // Step 3: fill image by copying lineBuffer x series height
        let currPos = cy * width * 4 * wy; // this is where we start filling image buffer with single series pixels
        for (let z = 0; z < wy; z++) {
          for (let lbp = 0; lbp < lineBufferLen; lbp++) {
            image.data[currPos++] = lineBuffer[lbp];
          }
        }
    ...
    // This render variant draws a rectangle on image data for each data point
    // Rect has width and height (like 10x10); wx calculated based on series length and screen width; wy is specified
    render_V1() {
      /*
      if (!this.data) {
        return;
      ...
      */
      ...
      // We draw on canvas with dimensions exactly as data dimensions
      ...
      let wx = Math.floor(width / serieslength);
      let wy = 20;
      let ww = wx * serieslength;
      height = numseries * wy;

      // wx calculation (rounding) will result in smaller width - so scale it up with css to full width on screen
      canvas.width = ww; //width; //W; // ?
      canvas.height = height; // H;
      canvas.style.width = `${width}px`;
      canvas.style.height = `${height}px`;
      let ctx = canvas.getContext('2d');
      ...
      for (let cy = 0, p = -1; cy < H; ++cy) {
        for (let cx = 0; cx < W; ++cx) {
          ...
          let c = d3.rgb(0, 0, 0);
          if (value !== 0) {
            c = d3.rgb(d3.interpolateYlOrRd(scaleValue(value)));
          }
          this.imgDataSetRect(image, c, cx, cy, wx, wy, width);
        }
      }
      ctx.putImageData(image, 0, 0);
      let endTs = Date.now();
    ...
    imgDataSetRect(image, c, cx, cy, wx, wy, w) {
      for (let y = 0; y < wy; y++) {
        let p = cy * w * wy * 4 + cx * wx * 4 + y * w * 4;
        for (let x = 0; x < wx; x++) {
          image.data[p++] = c.r;
          image.data[p++] = c.g;
          image.data[p++] = c.b;
